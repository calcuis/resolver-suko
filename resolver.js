...
function permutator(inputArr){var result=[];function permute(arr,m=[]){if(arr.length===0){result.push(m)}else{for(var i=0;i<arr.length;i++){var curr=arr.slice();var next=curr.splice(i,1);
...
cell.style.backgroundColor='RGB(136,201,125)';sums['n']=sums['n'].filter(item=>item!==selectedSquare);sums['m']=sums['m'].filter(item=>item!==selectedSquare);}});});
